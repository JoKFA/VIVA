...
-- Set user_role='admin' via a Supabase Database Function triggered on user creation,
-- or manually via: UPDATE auth.users SET raw_app_meta_data = raw_app_meta_data || '{"user_role":"admin"}' WHERE email = 'admin@vivahq.ca';
...
CREATE POLICY "public_read" ON stats FOR SELECT TO anon USING (published = true);
...
CREATE POLICY "public_read" ON programs FOR SELECT TO anon USING (published = true);
...
CREATE POLICY "public_read" ON events FOR SELECT TO anon USING (published = true);
...
CREATE POLICY "public_read" ON event_volunteer_contacts FOR SELECT TO anon USING (published = true);
...
CREATE POLICY "public_read" ON team_members FOR SELECT TO anon USING (published = true);
...
CREATE POLICY "public_read" ON awards FOR SELECT TO anon USING (published = true);
...
CREATE POLICY "public_read" ON annual_reports FOR SELECT TO anon USING (published = true);
...
CREATE POLICY "public_read" ON volunteer_roles FOR SELECT TO anon USING (published = true);
...
CREATE POLICY "public_read" ON partners FOR SELECT TO anon USING (published = true);
...
CREATE POLICY "public_read" ON testimonials FOR SELECT TO anon USING (published = true);
...
CREATE POLICY "public_read" ON faqs FOR SELECT TO anon USING (published = true);
...
CREATE POLICY "public_read" ON contact_info FOR SELECT TO anon USING (published = true);
...
CREATE POLICY "public_read" ON honours_carousel_entries FOR SELECT TO anon USING (published = true);
...
-- ─────────────────────────────────────────────────────────────
-- INSERT INTO storage.buckets (id, name, public) VALUES ('viva-media', 'viva-media', true)
-- ON CONFLICT (id) DO NOTHING;
--
-- Storage policies (in dashboard: Storage → Policies → viva-media):
-- anon:          SELECT on all objects
-- authenticated + is_admin(): INSERT, UPDATE, DELETE on all objects

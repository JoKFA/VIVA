...
export default function AdminTestimonials() {
  ...
  async function load() {
    const { data, error: err } = await supabase.from('testimonials').select('*');
    if (err) {
  ...
  async function save() {
    setError(null);
    let err;
    if (editing === 'new') ({ error: err } = await supabase.from('testimonials').insert([form]));
    else if (editing) ({ error: err } = await supabase.from('testimonials').update(form).eq('id', editing));
    if (err) {
      setError(err.message);
      return;
    }
  ...
  async function togglePublish(id: string, v: boolean) {
    setError(null);
    const { error: err } = await supabase.from('testimonials').update({ published: v }).eq('id', id);
    if (err) {
      setError(err.message);
      return;
    }
